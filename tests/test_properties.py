...
# Strategy for recurring patterns
@st.composite
def recurring_pattern_strategy(draw, is_income=False):
    ...
    return RecurringPattern(
        ...
        typical_day_of_month=next_date.day if is_income else None,
        flexibility="fixed", minimum_allowed_amount=None,
        representative_event_id="e1", last_date=date(2026, 9, 1),
        next_expected_date=next_date, is_salary=is_income
    )
...
def state_strategy(draw):
    ...
    expenses = [draw(recurring_pattern_strategy(is_income=False)) for _ in range(num_expenses)]
    
    num_incomes = draw(st.integers(min_value=0, max_value=2))
    incomes = [draw(recurring_pattern_strategy(is_income=True)) for _ in range(num_incomes)]
