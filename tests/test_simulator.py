...
from core.models import RecurringPattern, FinancialEvent
    ...
    state = FinancialState(
        ...
        recurring_expenses=[
            RecurringPattern(
                ...
                last_date=date(2026, 8, 16), next_expected_date=date(2026, 9, 15), is_salary=False
            )
        ]
    )
    
    days = simulate_cashflow(state)
    assert days[0].expenses == Decimal("500")
    assert days[0].closing_balance == Decimal("500")
    
    # Next should be 30 days later: 2026-10-15
    assert days[30].day == date(2026, 10, 15)
    assert days[30].expenses == Decimal("500")
